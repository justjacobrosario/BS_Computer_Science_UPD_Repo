...
: basically make an init function that returns initialized value for Model and calls function that gets the data for Cmd Msg

```elm
init : () -> (Model, Cmd Msg)
init _ = (init_val_of_Model, APIFetcher)
```
