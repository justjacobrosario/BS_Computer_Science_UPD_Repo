...
import pyxel
from constants import Key_Input, Tile, CELL_PX_SIZE
...
class Model:
    ...
    def move_player_dir(self):
        ...
        if Key_Input.GO_UP in self._curr_keys:
            if (0 <= (self._world.player.y - step) <= max_map_h):
                ...
                if next_block not in {Tile.BUSH.value, Tile.ROCK.value}:
                    self._world.player.y -= step
                elif next_block not in {Tile.BUSH.value, Tile.ROCK.value}:
    ...
    def upd_ingame_runtimes(self, key_inputs):
        self.upd_tick()
        self.upd_curr_keys(key_inputs)
        self.change_scale()

        self.is_quit()
        self.move_player_dir()

    def upd_cursor_coord(self, key_inputs, cursor_coords):
        self.curs_x, self.curs_y = cursor_coords
        if Key_Input.LEFT_CLICK in key_inputs:
