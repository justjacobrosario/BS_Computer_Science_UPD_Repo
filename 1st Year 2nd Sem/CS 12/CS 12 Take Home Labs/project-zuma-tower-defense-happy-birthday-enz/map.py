...
class Map:
    def __init__(self, width, height, data) -> None:
        ...
        path_file = data.get("path_maps", "path_maps.txt")
        paths_all_round, tunnel_all_round, guns_all_round = Map.parse_map_txt(path_file)
        ...
        self._transformed_gun_coords = (0, 0) # px coords of gun
        ...
        # Player Atrributes
        self._exp: int = 0 # ! Test value, should start with 0
        self._hp: int = data["player_lives"]
    ...
    def load_round_map(self, round_num):
        ...
        self._tunnel_paths = self._all_tunnel_paths.get(round_num, [])
        self._gun_coords = self._all_gun_coords.get(round_num, self._gun_coords)
        self.transform_gun_coords(*self._gun_coords)
    ...
    def check_if_next_round(self):
        round_done = (len(self._enemies[self._current_round - 1]) == 0) and (len(self._displayed_enemies) == 0)
        if not round_done:
            return
