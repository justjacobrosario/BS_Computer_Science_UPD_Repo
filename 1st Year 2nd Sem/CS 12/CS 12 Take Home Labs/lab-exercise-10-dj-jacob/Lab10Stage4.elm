  ...
  let
    ...
    updatedModel =
            case msg of
                ...
                MsgRemoveBox targetID -> 
                  if List.length model.boxes > 2 then
                    { model | boxes = List.filter (\box -> box.id /= targetID) model.boxes }
                  else
                    model
    allPairsPerBox : Int -> List String
    allPairsPerBox idx =
      let 
        currentBox = 
          updatedModel.boxes
            |> List.drop idx
            |> List.head
        remainingBoxes = List.drop (idx + 1) updatedModel.boxes
      in
      case currentBox of
        Just box -> List.map (\otherBox -> checkStrings updatedModel.dropbox box otherBox) remainingBoxes
        Nothing -> []
    newOutput = List.foldl (\idx acc -> acc ++ (allPairsPerBox idx)) [] (List.range 0 ((List.length model.boxes) - 1))
  in
...
view model =
  let
    ...
    convertBoxToInput box =
      div [] [ input [onInput (MsgBox box.id), value box.content] [], button [onClick (MsgRemoveBox box.id)] [text "Remove name"] ]
